...
#             train_running_loss=train_running_loss+(-(y[0,n]*np.log(layers_output[-1])+(1-y[0,n])*np.log(1-layers_output[-1])))
            ...
            derivatives=cal_derivatives(layers_output, weights, b,layers, 'sigmoid')
...
#         val_corrects=np.sum(y_val == val_output).astype('float')
    ...
    plt.title('model accuracy for MNIST with sigmoid and softmax fold {}'.format(f+1))
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    plt.legend(['train_acc', 'val_acc'], loc='upper left')
    fig.savefig('./fold_' + str(f+1) + '_sigmoid_softamx_accuracy.png')
    plt.show()
...
np.savez('weights_MNIST_sig', weights)

...
data, lab=read_data('dataset_partA.h5')
...
#     fig=plt.figure(figsize=(8,10))
    ...
    for e in range(epoch):
        ...
        for n in range(x.shape[1]):
            ...
            derivatives=cal_derivatives(layers_output, weights, y[0,n],layers, 'sigmoid')
        ...
        print('fold {}: epoch {}/{}: train_accuracy {}: val_accuracy {}: train_loss {}'.format(f+1, e,epoch-1,accuracy[e,0], val_accuracy[e,0], train_loss[e,0]))

    fig=plt.figure(figsize=(8,6))
    plt.style.use('bmh')
    plt.plot(epochs,accuracy,'r', epochs, val_accuracy, 'g')
    plt.title('model accuracy for partA with sigmoid fold {}'.format(f+1))
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    plt.legend(['train_acc', 'val_acc'], loc='upper left')
    fig.savefig('./fold_' + str(f+1) + '_partA_sigmoid_accuracy.png')
    plt.show()
...
np.savez('weights_partA_sig', weights)
